...
- [x] **Cache Components migration — blockers documented** (May 22, 2026, commit `20359c2`) — trial enable surfaced ~20 files that need coordinated changes (11 `revalidate=N` pages, 5 `runtime='edge'` routes, 2 dynamic-API pages without Suspense). Reverted to off; `docs/ENGINEERING.md` §14 captures the reproducible audit + migration order. Reactivate when ready to commit to the coordinated pass.
...
- [ ] **Execute Cache Components migration** — coordinated pass per `docs/ENGINEERING.md` §14: convert 11 marketing pages from `revalidate=N` to `"use cache" + cacheLife()`, audit edge routes for Node migration, Suspense-wrap dynamic API reads on `/` and `/admin/marketing`. Single PR; verify in preview before flipping the production flag.
