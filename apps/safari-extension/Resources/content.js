/**
 * COYL Safari Web Extension — content script
 *
 ...
 *
 * Same DOM logic as the Chrome build — DOM is DOM. Uses the
 * WebExtensions standard `browser.*` namespace (falls back to
 * `chrome` for safety).
 *
...
function handleAction(action, host) {
  switch (action) {
    ...
    case 'open-rescue':
      sendFeedback('helpful', host)
      ext.runtime.sendMessage({ type: 'COYL_OPEN_RESCUE' })
      dismissOverlay()
      break
    case 'continue-1min':
      ...
      break
    case 'mute-1h':
      ext.runtime.sendMessage({ type: 'COYL_MUTE', host, duration: '1h' })
      dismissOverlay()
      break
    case 'mute-1d':
      ext.runtime.sendMessage({ type: 'COYL_MUTE', host, duration: '1d' })
      dismissOverlay()
      break
    case 'mute-forever':
      ext.runtime.sendMessage({ type: 'COYL_MUTE', host, duration: 'forever' })
      dismissOverlay()
      break
  }
}
...
function sendFeedback(verdict, host) {
  ext.runtime.sendMessage({ type: 'COYL_FEEDBACK', verdict, host, timestamp: Date.now() })
}
