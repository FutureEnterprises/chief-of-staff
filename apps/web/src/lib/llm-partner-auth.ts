/**
 * LLM-partner Bearer-token authentication.
 *
 * Every incoming PAP/EAP request from a foundation lab carries an
 * Authorization header of the form:
 *
 *   Authorization: Bearer coyl_pap_<llmPartnerId>_<keySecret>
 *
 * authenticateLLMPartner() splits that token into its three parts,
 ...
 *
 * Failed-auth attempts are recorded in EAPAuditEntry so the admin
 * dashboard can surface credential-stuffing patterns. We rate-limit
 * the audit writes (one entry per minute per IP) so a high-volume
 * attacker can't blow up our audit table.
 */
...
/** Token prefix that every issued partner key carries. */
...
/** Result envelope for authenticateLLMPartner — either a partner or a 401 response. */
export type LLMPartnerAuthResult =
 ...
 */
export async function authenticateLLMPartner(req: Request): Promise<LLMPartnerAuthResult> {
 ...
 * throws an LLMPartnerAuthError carrying the 401 NextResponse to be
 * caught by the route handler. Use in routes that don't want to
 * destructure the result object.
 */
export async function requireLLMPartner(req: Request): Promise<LLMPartner> {
  const result = await authenticateLLMPartner(req)
  if (result.error) throw new LLMPartnerAuthError(result.error)
  return result.partner
}

export class LLMPartnerAuthError extends Error {
  readonly response: NextResponse
  constructor(response: NextResponse) {
    super('LLMPartner authentication failed')
    this.name = 'LLMPartnerAuthError'
    this.response = response
  }
}

/**
 * Best-effort EAPAuditEntry write for a failed auth attempt.
...
async function recordFailedAuth(
  ...
  try {
    ...
    console.warn('[llm-partner-auth] failed to write audit entry', {
