    ...
    <>
      ...
      <section className="mb-16 grid grid-cols-1 gap-4 md:grid-cols-2">
        {[
          ...
          <div key={p.title} className="rounded-2xl border border-gray-200 bg-gradient-to-br from-red-500/5 to-transparent p-5">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-gray-700">
          {['Cue', 'Urge', 'Excuse', 'Lapse', 'Shame', 'Hiding', 'Repeat'].map((node, i) => (
            <span key={node} className="flex items-center gap-3">
              <span className="rounded-full border border-red-500/30 bg-gray-100 px-3 py-1.5 font-mono">{node}</span>
      ...
      <Link href="/sign-up?wedge=destructive-behaviors" className="inline-block rounded-full bg-gradient-to-r from-red-500 to-orange-500 px-6 py-3 text-sm font-bold text-gray-900">
        Map my loop
      </Link>
    </>
  )
