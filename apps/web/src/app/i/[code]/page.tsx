...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const ogImage = `/api/og?title=${encodeURIComponent('COYL caught me.')}&kicker=${encodeURIComponent(`${card.localTimeLabel} · ${card.triggerLabel}`)}`
      ...
      <div className="mx-auto max-w-2xl px-6 py-10 md:py-16">
        ...
        <section className="mt-12 rounded-3xl border border-white/10 bg-white/[0.02] p-6 md:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/sign-up?ref=share"
              className="rounded-full bg-gradient-to-r from-orange-500 to-red-500 px-5 py-2.5 text-sm font-bold text-white shadow-[0_0_18px_rgba(255,102,0,0.3)]"
            >
