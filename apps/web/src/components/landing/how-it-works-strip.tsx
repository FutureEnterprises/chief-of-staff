  ...
  return (
    <section className="relative mx-auto max-w-6xl px-6 py-24 md:py-32">
      ...
      <h2 className="mb-4 max-w-3xl text-4xl font-black leading-[1.05] tracking-tight text-white md:text-6xl">
        Three steps.<br />
        <span className="text-orange-400">No restart, no shame, no streaks to protect.</span>
      </h2>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {steps.map((s, i) => (
          <motion.div
            ...
            className="rounded-3xl border border-white/5 bg-gradient-to-br from-white/[0.03] to-transparent p-8"
          >
            <div className="mb-6 flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-orange-500 tracking-widest">
                {s.n}
              </span>
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-orange-500/20 bg-orange-500/[0.08] text-orange-400">
                <s.Icon className="h-5 w-5" />
              </div>
            </div>
            <h3 className="mb-3 text-2xl font-black text-white">{s.title}</h3>
            <p className="text-base leading-relaxed text-gray-400">{s.body}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="mt-12 flex flex-wrap gap-3">
        <Link
          href="/how-it-works"
          className="rounded-full border border-white/10 bg-white/5 px-6 py-3 text-sm font-semibold text-gray-200 transition-colors hover:border-orange-500/40 hover:text-white"
        >
          See the full loop
        </Link>
        <Link
