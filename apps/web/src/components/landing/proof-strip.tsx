    ...
    <section ref={ref} className="relative mx-auto max-w-6xl px-6 py-24 md:px-12">
      ...
      >
        ...
        <h2 className="text-4xl font-black leading-[1.05] tracking-tight text-white md:text-5xl">
          ...
          <span className="text-orange-400">nobody else fights for.</span>
        </h2>
      </motion.div>
      ...
      <motion.div
        ...
        className="overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent"
      >
        <div className="grid grid-cols-[1.2fr_2fr_2fr_1fr] gap-0 border-b border-white/5 px-5 py-3 text-[10px] font-mono uppercase tracking-[0.22em] text-gray-500">
          <div></div>
        ...
        {comparison.map((row) => (
          <div
            key={row.product}
            className={`grid grid-cols-[1.2fr_2fr_2fr_1fr] items-center gap-0 border-b border-white/5 px-5 py-4 text-sm last:border-b-0 ${
              row.highlight ? 'bg-orange-500/[0.04]' : ''
            }`}
          >
            <div className={`font-bold ${row.highlight ? 'text-orange-300' : 'text-foreground'}`}>
              {row.product}
            </div>
            ...
            <div className="text-gray-400">{row.shame}</div>
            <div className="text-right font-mono text-gray-300">{row.price}</div>
          </div>
        ))}
      </motion.div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-3 md:grid-cols-2">
        <motion.div
          ...
          className="rounded-2xl border border-orange-500/30 bg-orange-500/[0.05] p-5"
        >
          ...
          <Link
            href="/clinical-study"
            className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-orange-300 hover:text-orange-200"
          >
